...
const props = defineProps({ lines: { type: Array, default: () => [] }, compact: Boolean })
...
<template>
 <div class="raw-console" :class="{compact}">
  <div class="console-tools"><span><i class="dot"/> 后台输出 <small>演示日志</small></span><div><label><input type="checkbox" v-model="wrap"/> 自动换行</label><label><input type="checkbox" v-model="follow"/> 自动滚动</label><button @click="copy">{{copied?'已复制':'复制日志'}}</button></div></div>
  ...
  <div class="console-footer"><span>{{lines.length}} 行 · {{follow?'跟随最新输出':'已暂停自动滚动，可向上查看'}} </span><button @click="follow=true;bottom()">跳到最新 ↓</button></div>
 </div>
</template>
