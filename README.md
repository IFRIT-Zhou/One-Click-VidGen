...
OCV 根目录的 [`plugins`](./plugins) 文件夹用于放置社区扩展。当前版本只扫描并展示
`plugin.json` 清单，不会导入或执行第三方代码；用户可以在左侧“扩展与插件”页面查看清单、
记录启用状态、重新扫描或打开插件目录。Launcher 安全更新会保留所有第三方插件文件。

后续计划优先开放 ComfyUI/图像模型适配器，再逐步提供 TTS、Agent、提示词处理和渲染后处理
扩展点。第三方插件不代表 OCV 官方审核、担保或授权，插件应单独说明依赖、权限、许可证和
数据处理行为。
