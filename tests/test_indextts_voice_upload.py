...
from backend.app import indextts25_local
...
class IndexTtsVoiceUploadTest(unittest.TestCase):
    def test_uploaded_reference_audio_is_resolved_inside_user_directory(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            with patch.object(indextts25_local, "PROJECT_ROOT", root):
                resolved = indextts25_local.resolve_voice_reference(
                    config,
    ...
    def test_uploaded_reference_rejects_path_traversal(self) -> None:
        ...
        with self.assertRaises(ValueError):
            indextts25_local.resolve_voice_reference(
                config,
                "upload:../voice.wav",
                user_id=7,
            )
